...
class KNN_Model(KNN_ConfMtx):
    ...
    def get_neighbor_counts(self, new_image, return_indices=False):
        ...
        #This uses broadcasting to take the eucliden distance between the new_imahe
        #and every other image. It takes the square distance, then sums each of the values

        distances = np.square(self.image_train - new_image)
        ...
        #Uses np.argsort() to return a sorted list of indicies, then python slicing
        #to return the k nearest indicies. Then uses the indicies to return the 
        #k nearest labls
        ...
        class_counts = [np.sum(nearest_label == j)for j in self.class_list]
    ...
    def predict(self, new_image):
        ...
        class_counts_array= self.get_neighbor_counts(new_image)
        prediction_label = self.class_list[class_counts_array.index(max(class_counts_array))]
    ...
    def get_prediction_array(self, image_test):
        ...
        #For each image, predict the output and add it to the predicition list
        #Print iteration multiples of 10
