...
def get_data_MNIST(subset, data_path="../data"):
    ...
    def decode(filepath, count, offset):
        with open(filepath, "rb") as f, gzip.GzipFile(fileobj= f) as bytestream:
            return np.frombuffer(bytestream.read(), dtype = np.uint8, count = count, offset = offset)
    

    ## TODO: read the image file and normalize, flatten, and type-convert image
    image = decode(inputs_file_path, num_examples * 784, 16)
    image = np.reshape(image, (num_examples, -1)) / 255
    image = image.astype(np.float32)

    label = decode(labels_file_path, num_examples, 8)
...
def get_data_CIFAR(subset, data_path="../data"):
    ...
    for file in data_files:
        with open(file, "rb") as fo:
            cifar_dict_batch = pickle.load(fo, encoding="bytes")
            cifar_dict[b"data"] += list(cifar_dict_batch[b"data"])
            cifar_dict[b"labels"] += list(cifar_dict_batch[b"labels"])
    ...
    label_names = np.array(label_names, dtype="U")
...
def get_specific_class(image_full, label_full, specific_class=0, num=None):
    ...
    #Use logical indexing (mask) to return the items of the list for a specific
    #class
...
def get_subset(image_full, label_full, class_list=list(range(10)), num=100):
    ...
    for x in class_list:
        images, labels = get_specific_class(image_full, label_full, specific_class=x, num=num)
